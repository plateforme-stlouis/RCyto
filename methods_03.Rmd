...
fs <- FlowSOM(ff, compensate = F,transform = F, scale = F,
                colsToUse = colnames(ff@exprs), nClus = 10, seed = 42)
...
In this example, the cluster 1 achieves a maximum F1 score when it is matched to the population 19. 
...
In this example:  
* The maximum F1 score for population 1 is achieved when this population is matched to cluster 10.
* Many populations achieve their maximum F1 score when they are matched to cluster 2.
...
Notice that with a less restrictive trheshold, some populations assigned to cluster 2 could have been merged.
