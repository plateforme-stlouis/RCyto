...
subtitle: "Experience `real_02`, sample `5` (`Temoin 4`), downsampled"
output:
  html_document:
    number_sections: yes
    toc: yes
...
kable(summ_mains[,3:6], digits = 3)
```

```{r, eval = T}
# export results (mean F1)
write.table(summ_mains, 
            file = "results/summ_mains.txt", append = T, sep = "\t", row.names = F, col.names = F)
```
...
```{r, eval = T}
# export plot
png(paste("results/graph_MAINS_", params$file, ".png", sep = ""), res=150, width = 1400, height = 1400)
ggplot(df, aes(class, F1)) + geom_point(aes(size = frequencies, color = frequencies)) + 
...
write.table(summ_subs, 
            file = "results/summ_subs.txt", append = T, sep = "\t", row.names = F, col.names = F)
```
...
```{r, eval = T}
# export plot
png(paste("results/graph_SUBS_", params$file, ".png", sep = ""), res=150, width = 1400, height = 2400)
ggplot(df, aes(class, F1)) + geom_point(aes(color = main_class)) + 
