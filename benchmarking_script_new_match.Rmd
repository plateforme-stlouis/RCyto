...
matching <- function(clustering, labels){
    ...
    for(j in 1:length(row_m)){ 
      if(clustering[i] == names(row_m)[j]){ # if cluster numbers equals a row maximum...
        ...
        if(clustering[i] %in% unlist(col_m)){ # # ... that it is also a column maximum...
          ...
          matched_merged_clusters[i] <- levels(labels)[m[[j]]]
        }
      }
    }
  }
  ...
  # Number of MERGED PARTITIONS (merged clusters)
  merged_partitions <- length(table(matched_merged_clusters))
  ...
  matched <- list("c" = c, "f" = f, "m" = m, "merged_labels" = merged_labels,
                  ...
                  "partitions" = partitions, "merged_partitions" = merged_partitions)
  return(matched)
}
```
...
```{r f1}
mean_f1 <- function(cm, cm_merged, merged_labels){
  # NOT MERGED, FOUND
  f1_list <- cm$byClass[,"F1"]
  f1_no_nas <- f1_list[!is.na(f1_list)]   # removing NAs
  mf1_f <- mean(f1_no_nas)
  
  pop_freq <- prop.table(table(labels))
  pop_freq_no_nas <- pop_freq[!is.na(f1_list)] / sum(pop_freq[!is.na(f1_list)])
  mf1_f_w <- t(pop_freq_no_nas) %*% f1_no_nas # WEIGHTED MEAN 
  
  # NOT MERGED, ALL
  f1_zeros <- f1_list
  f1_zeros <- ifelse(is.na(f1_list), 0, f1_list)
  mf1_all <- mean(f1_zeros)
  mf1_all_w <- t(pop_freq) %*% f1_zeros # WEIGHTED MEAN 
  
  # MERGED, FOUND
  f1_list <- cm_merged$byClass[,"F1"]
  f1_no_nas <- f1_list[!is.na(f1_list)]   # removing NAs
  mf1_m_f <- mean(f1_no_nas)
  
  pop_freq <- prop.table(table(merged_labels))
  pop_freq_no_nas <- pop_freq[!is.na(f1_list)] / sum(pop_freq[!is.na(f1_list)])
  mf1_m_f_w <- t(pop_freq_no_nas) %*% f1_no_nas # WEIGHTED MEAN 
  
  # MERGED, ALL
  f1_zeros <- f1_list
  f1_zeros <- ifelse(is.na(f1_list), 0, f1_list)
  mf1_m_all <- mean(f1_zeros)
  mf1_m_all_w <- t(pop_freq) %*% f1_zeros # WEIGHTED MEAN 
  
  f1 <- list("found" = mf1_f, "w_found" = mf1_f_w, 
             "all" = mf1_all, "w_all" = mf1_all_w,
             "merged_found" = mf1_m_f, "merged_w_found" = mf1_m_f_w, 
             "merged_all" = mf1_m_all, "merged_w_all" = mf1_m_all_w)
  return(f1)
}
```
...
```{r table}
pops_f1 <- c()
summ <- data.frame(File = NA, n = 0, Method = NA, Clusters = 0, 
                   Partitions = 0, Merged_partitions = 0,
                   Mean_F1_found = 0, Weighted_mean_F1_found = 0, 
                   Mean_F1_all = 0, Weighted_mean_F1_all = 0,
                   Mean_F1_merged_found = 0, Weighted_mean_F1_merged_found = 0, 
                   Mean_F1_merged_all = 0, Weighted_mean_F1_merged_all = 0,
...
for(i in 1:length(clusters)){
  ...
  mf1 <- mean_f1(cm, cm_merged, matched$merged_labels)
  # Storing the result 
  summ[i,c(1,3)] <- c(params$file, method)
  summ[i,-c(1,3)] <- c(nrow(ff@exprs), clusters[i],
                       matched$partitions, matched$merged_partitions,
                       mf1$found, mf1$w_found, mf1$all, mf1$w_all, 
                       mf1$merged_found, mf1$merged_w_found, mf1$merged_all, mf1$merged_w_all,
